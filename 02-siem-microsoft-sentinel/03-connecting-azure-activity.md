...
workspace ("SocAnalyst") as my first data source.
...
## Troubleshooting: the diagnostic settings mix-up
This was the trickiest part. Azure has multiple pages that all look similar 
...
- **What worked:** I found the correct path by going to the Activity Log page 
  directly and using the "Export Activity Logs" button in the TOP toolbar 
  (not the left sidebar). This opened a diagnostic settings page scoped 
  specifically to the subscription's Activity Log, where I could route it to 
  my "egom" Log Analytics workspace.
...
## Verification
Ran the query `AzureActivity | take 10` in Sentinel Logs to confirm data was 
flowing after setting up the diagnostic setting.

## Status
🟡 In progress — diagnostic setting saved, waiting for data to populate
